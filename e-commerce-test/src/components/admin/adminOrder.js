...
const AdminOrder = () => {
    ...
    const handleStatusSort = async (status) => {
        try {
            const res = await apiRequest.get(`admin/order/purchases`);
    ...
    return (
        <div className="adminPanelOrders">
            <div className="header-admin-panel">
                <div className="header-admin-panel-title">Управление заказами</div>
                <svg
                    width="200"
                    height="75"
                    viewBox="0 0 996 398"
            ...
            <div className="adminPanel-orders">
                <h1>Заказы</h1>
                {orderItems ? (
                    <div className="sortButtons">
                        <button
                            className={`sortOrder ${priceSortOrder === "asc" ? "active" : ""}`}
                ...
                <div className="adminPanel-testFlex">
                    {orderItems.length > 0 ? (
                        orderItems.map((order) => (
                            <div
                                key={order.id}
                                className="admin-order-flex"
